...
#  TODO  跳转企税季报机器人基础配置页
def corporate_tax_quarterly_report_robot_base(request):
...
def corporate_tax_quarterly_report_robot_business(request):
...
def corporate_tax_quarterly_report_robot_jobs(request):
    user_name = request.COOKIES.get('user_name')
    # update_sql(user_name)
...
def corporate_tax_quarterly_report_requisition_1(request):
    return render(request, 'corporate_tax_quarterly_report_requisition_1.html')
