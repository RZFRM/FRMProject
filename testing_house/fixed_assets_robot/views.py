...
def purchaes_requisitions_create_data(request):
    ...
    try:
        DB.get_insert(table='FA_purchase_requisition_table',
    ...
    try:
        ...
        job_list_summary.job_type = '固定资产采购申请机器人'
...
def set_purchaes_order_create_data(request):
    ...
    sql = "select  goods_number  from FA_purchase_requisition_table  where  purchase_number = '%s'" % purchase_number
    ...
    try:
        DB.get_insert(table='FA_purchase_order_table',
    ...
    try:
        job_list_summary = Job_list_summary()
        job_list_summary.job_type = '固定资产采购合同机器人'
        job_list_summary.job_no = job_no
